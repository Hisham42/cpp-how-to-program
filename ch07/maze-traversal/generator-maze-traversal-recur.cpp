...
void mazeGenerator(int start, int end);
bool randomMove();
...
void displayMaze(char (*arr)[mazeRows]);
    ...
    uniform_int_distribution <unsigned int> randomNum(1, 10);
...
int main() {
    ...
    int x = randomNum(engine);
    
    
    mazeGenerator(3, randomNum(engine));
...
void mazeGenerator(int start, int end) {
    ...
    {
        for (size_t j = 1; j < mazeColumns - 1; j++)
        {
            if (randomMove())
            {
                maze[i][j] = '.';
...
bool randomMove() {
    ...
    int x{0};
    int y{0};
    uniform_int_distribution <unsigned int> randomMove(0, 1);
